...
public class BoardMgr {
	...
	public void insertBoard(HttpServletRequest req) {
		...
		try {
			...
			if(!dir.exists()/*존재하지 않는다면*/)
				dir.mkdirs();//상위폴더가 없어도 생성을 함
				//mkdir은 상위폴더가 없으면 생성불가
	...
	//Board List : 검색기능, 페이징 및 출력
	//limit 시작번호(int start), 가져올 개수(int cnt)
	//검색했을 때랑 안했을 때랑 나와야해서 keyField랑 keyWord 사용해줌.
	...
	//파일일 업로드 수정이 되면 기존에 파일은 삭제가 되어야 한다.
	//기존의 파일이 있지만 파일 업로드 수정이 없으면 그냥 다른 컬럼들만 수정
